...
module State =
    ...
    let init () =
        ...
        let implicitCanvasAnimationsModel, implicitCanvasAnimationsCmdMsgs =
            ImplicitCanvasAnimationsPage.init()

        let compositorAnimationsModel, compositorAnimationsCmdMsgs =
            CompositorAnimationsPage.init()
        ...
        let drawLineAnimationModel, drawLineAnimationCmdMsgs = DrawLineAnimationPage.init()
        ...
        let lineBoundsDemoControlModel, lineBoundsDemoControlCmdMsgs =
            LineBoundsDemoControlPage.init()
        ...
        let transitionsModel, transitionsCmdMsgs = TransitionsPage.init()
        let transformsModel, transformsCmdMsgs = TransformsPage.init()
        ...
        { AcrylicPageModel = acrylicModel
          ...
          AnimationsPageModel = animationsModel
          ImplicitCanvasAnimationsPageModel = implicitCanvasAnimationsModel
          CompositorAnimationsPageModel = compositorAnimationsModel
          ButtonsPageModel = buttonsModel
          BrushesPageModel = brushesModel
          ...
          ClippingPageModel = clippingModel
          ...
          DrawingPageModel = drawingModel
          DrawLineAnimationPageModel = drawLineAnimationModel
          ...
          LineBoundsDemoControlPageModel = lineBoundsDemoControlModel
          ...
          TransitionsPageModel = transitionsModel
          TransformsPageModel = transformsModel
        ...
        [ SubpageCmdMsgs acrylicCmdMsgs
          ...
          SubpageCmdMsgs implicitCanvasAnimationsCmdMsgs
          SubpageCmdMsgs compositorAnimationsCmdMsgs
          ...
          SubpageCmdMsgs drawLineAnimationCmdMsgs
          ...
          SubpageCmdMsgs lineBoundsDemoControlCmdMsgs
          ...
          SubpageCmdMsgs transitionsCmdMsgs
          SubpageCmdMsgs transformsCmdMsgs
    ...
    let update msg model =
        ...
        | Update date ->
            model, [ NewMsg(CanvasPageMsg(CanvasPage.Msg.Update(date))) ]
        ...
        | AnimationsPageMsg msg ->
            let model1, cmdMsgs = AnimationsPage.update msg model.AnimationsPageModel

            { model with
                AnimationsPageModel = model1 },
            [ SubpageCmdMsgs cmdMsgs ]
        | ImplicitCanvasAnimationsPageMsg msg ->
            let model1, cmdMsgs =
                ImplicitCanvasAnimationsPage.update msg model.ImplicitCanvasAnimationsPageModel
            //let cmdMsgs = [ Cmd.map SubpageCmdMsgs  cmdMsgs ]
            { model with
                ImplicitCanvasAnimationsPageModel = model1 },
            [ SubpageCmdMsgs [ (ImplicitCanvasAnimationsPageCmdMsgs cmdMsgs) ] ]
        | CompositorAnimationsPageMsg msg ->
            let model1, cmdMsgs =
                CompositorAnimationsPage.update msg model.CompositorAnimationsPageModel

            { model with
                CompositorAnimationsPageModel = model1 },
            [ SubpageCmdMsgs cmdMsgs ]
        | ButtonsPageMsg msg ->
        ...
        | BrushesPageMsg msg ->
            let model1, cmdMsgs = BrushesPage.update msg model.BrushesPageModel
            { model with BrushesPageModel = model1 }, [ SubpageCmdMsgs cmdMsgs ]
        ...
        | ContextFlyoutPageMsg msg ->
            ...
            [ SubpageCmdMsgs cmdMsgs ]
        | ClippingPageMsg msg ->
            let model1, cmdMsgs = ClippingPage.update msg model.ClippingPageModel

            { model with
                ClippingPageModel = model1 },
            [ SubpageCmdMsgs cmdMsgs ]
        ...
        | DrawLineAnimationPageMsg msg ->
            let model1, cmdMsgs =
                DrawLineAnimationPage.update msg model.DrawLineAnimationPageModel

            { model with
                DrawLineAnimationPageModel = model1 },
            [ SubpageCmdMsgs [ DrawLineAnimationPageCmdMsgs cmdMsgs ] ]
        | DrawingPageMsg msg ->
            let model1, cmdMsgs = DrawingPage.update msg model.DrawingPageModel
            { model with DrawingPageModel = model1 }, [ SubpageCmdMsgs cmdMsgs ]
        | EffectsPageMsg msg ->
        ...
        | LineBoundsDemoControlPageMsg msg ->
            let model1, cmdMsgs =
                LineBoundsDemoControlPage.update msg model.LineBoundsDemoControlPageModel

            { model with
                LineBoundsDemoControlPageModel = model1 },
            [ SubpageCmdMsgs cmdMsgs ]
        | ListBoxPageMsg msg ->
        ...
        | TransitionsPageMsg msg ->
            let model1, cmdMsgs = TransitionsPage.update msg model.TransitionsPageModel

            { model with
                TransitionsPageModel = model1 },
            [ SubpageCmdMsgs cmdMsgs ]
        | TransformsPageMsg msg ->
            let model1, cmdMsgs = TransformsPage.update msg model.TransformsPageModel

            { model with
                TransformsPageModel = model1 },
            [ SubpageCmdMsgs cmdMsgs ]
